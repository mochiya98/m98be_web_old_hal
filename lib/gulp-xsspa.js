...
gulp_xsspa_builder.prototype.updateBase = function(){
	...
	let base_html = fs.readFileSync(opts.base)
		.toString();
	let regex_contents = /<%= *contents? *%>/ig;
	let regex_title = /<%= *title? *%>/ig;
	...
	t.p2 = Buffer.from(base_html.slice(regex_title.lastIndex, match_contents.index));
...
gulp_xsspa_builder.prototype.compile = function(opts){
	...
	let transform = function(file, encoding, callback){
		if(file.isNull()){
			...
			if(file.build_conf.title) title_buf = Buffer.from(`${file.build_conf.title} - ${t.opts.base_title}`);
			file.contents = Buffer.concat([t.p1, title_buf, t.p2, file.contents, t.p3]);
			return callback(null, file);
		}else{
			this.push(file);
		}
