	...
	let transform = function(file, encoding, callback) {
		if (file.isNull()) {
			...
			try {
				...
				if (build_conf.template === void 0) {
					this.emit(
						"error",
						new PluginError(PLUGIN_NAME, "build_conf.template is undefined!"),
					);
					return callback();
				}
				if (build_conf.data === void 0) {
					this.emit(
						"error",
						new PluginError(PLUGIN_NAME, "build_conf.data is undefined!"),
					);
					return callback();
				}
				...
				if (opts.outputExtension) {
					...
					file.path = path.join(
						parsed_path.dir,
						parsed_path.name + "." + opts.outputExtension,
					);
				}
				return callback(null, file);
			} catch (e) {
...
gulp_xste.bundle = function(opts) {
